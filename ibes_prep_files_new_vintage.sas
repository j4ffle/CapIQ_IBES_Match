...
data epsafy; set ibes.det_epsus (keep = ticker analys anndats);
year = year(anndats);
run;
...
*Get gvkey;
proc sql; create table ibesafy7 as select distinct
	a.*, b.gvkey 
    from ibesafy6 as a left join crsp.ccmxpf_linktable
...
proc download data = ibesafy8; run;
endrsubmit;
